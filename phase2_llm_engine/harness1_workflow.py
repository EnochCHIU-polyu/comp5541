...
from phase2_llm_engine.trackb_harnesses.harness1.h1_1 import build_retrieval_context, retrieve_chunks
from phase2_llm_engine.trackb_harnesses.harness1.h1_2 import build_numeric_hint, numeric_guard
from phase2_llm_engine.trackb_harnesses.harness1.h1_3 import chronology_guard
from phase2_llm_engine.trackb_harnesses.harness1.h1_4 import verify_support
...
def _build_batch_prompt(payloads: list[dict[str, Any]]) -> list[dict[str, str]]:
    sections: list[str] = []
    for item in payloads:
        hint_block = item.get("hint_block", "")
        sections.append(
            "\n".join(
                [
                    f"CASE_ID: {item['case'].case_id}",
                    f"Question:\n{item['case'].question}",
                    f"{hint_block}\n" if hint_block else "",
                    f"Context:\n{item['context']}",
                ]
    ...
    return [
        {
            ...
            "content": (
                "You are a financial audit assistant. "
                "Answer each case independently in English. "
                "Prefer exact table rows or disclosure lines, not rounded narrative summaries. "
                "Return strict blocks in this format for every case id:\n"
                "CASE_ID: <id>\n"
                "ANSWER: <short answer>\n"
                "CITATIONS: <semicolon-separated quoted evidence snippets>"
            ),
...
def _parse_batch_answers(raw: str) -> dict[str, tuple[str, list[str]]]:
    rows: dict[str, tuple[str, list[str]]] = {}
    blocks = re.split(r"(?im)^\s*case_id\s*:\s*", raw)
    for block in blocks[1:]:
        lines = block.splitlines()
        if not lines:
            continue
        case_id = lines[0].strip().split()[0]
        body = "\n".join(lines[1:]).strip()
        answer, citations = _parse_answer(body)
        if case_id:
            rows[case_id] = (answer, citations)
    return rows
...
def _prepare_case_payload(
    ...
    use_h1_retrieval: bool,
    use_h2_numeric_guard: bool,
) -> dict[str, Any]:
    if use_h1_retrieval:
...
def _build_result(
    ...
    raw_response: str,
    use_h2_numeric_guard: bool,
    use_h3_chronology_guard: bool,
    use_h4_verifier: bool,
) -> FinancialWorkflowResult:
    case: FinancialEvalCase = payload["case"]
    diagnostics: dict[str, Any] = {
        "retrieved_chunks": len(payload["chunks"]),
    }
    ...
    if use_h4_verifier:
        diagnostics["verification"] = verify_support(
            answer,
            citations,
            report_text,
            evidence_keywords=case.evidence_keywords,
...
def run_financial_workflow(
    ...
    payload = _prepare_case_payload(report_text, case, use_h1_retrieval, use_h2_numeric_guard)
    messages = _build_prompt(case.question, payload["context"], hint_block=payload["hint_block"])
    ...
    if use_h4_verifier:
        initial_verification = result.diagnostics["verification"]
        if not initial_verification.get("verified", False):
            revision_context = payload["context"]
            revision_prompt = [
                {
                    "role": "system",
                    "content": (
            ...
            result.diagnostics["verification"] = revised_verification
...
def run_financial_workflow_batch(
    ...
    payloads = [
        _prepare_case_payload(report_text, case, use_h1_retrieval, use_h2_numeric_guard)
        for case in cases
    ]
    raw = query_llm(_build_batch_prompt(payloads), model=model, temperature=temperature)
    parsed = _parse_batch_answers(raw)

    results: list[FinancialWorkflowResult] = []
    for payload in payloads:
        case: FinancialEvalCase = payload["case"]
        answer, citations = parsed.get(case.case_id, ("", []))
        result = _build_result(
            report_text,
            payload,
            answer,
        ...
        if use_h4_verifier and not result.diagnostics["verification"].get("verified", False):
            # Preserve low request count in batch mode by avoiding per-case revision calls.
            result.diagnostics["verification"]["revision_skipped_due_batch"] = True
