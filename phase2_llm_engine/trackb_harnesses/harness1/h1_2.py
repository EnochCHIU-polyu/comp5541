"""h1_2 - Numeric/unit guard component for Harness 1 (Track B)."""
...
def _unit_bucket(text: str) -> str:
    low = text.lower()
    if "billion" in low or "十亿" in low or "亿元" in low:
        return "billion"
    ...
    if case.answer_type == "numeric":
        if pred is None or exp is None:
            ...
            if not numeric_match:
                expected_pct = "%" in case.expected_answer or "percent" in case.expected_answer.lower()
                answer_pct = "%" in answer or "percent" in answer.lower()
                if expected_pct or answer_pct:
                    if abs(pred / 100.0 - exp) <= max(case.tolerance, 1e-9):
                        numeric_match = True
                    elif abs(pred - exp * 100.0) <= max(case.tolerance, 1e-9):
                        numeric_match = True
